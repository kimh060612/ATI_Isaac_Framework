...
class L2DiscreteDisjointLinUCBRGBCamPolicy(BaseCMABPolicy):
    """
    Disjoint Linear UCB policy for RGB camera control w/ Discrete Context. Controlling only Exposure time and ISO with discrete actions.
    ...
    """
    ANGULAR_SPEED_THRESHOLDS = (2.5, 5.0, 7.5, 10.0, 12.5)
    LIGHT_INTENSITY_THRESHOLDS = (500.0, 1000.0, 3000.0, 6000.0)
    ...
    def __init__(
        ...
        # Feature dimension:
        # [1, one_hot(abs(w) bin), one_hot(light bin)]
        dim_context = (
            1
            + len(self.ANGULAR_SPEED_THRESHOLDS)
            + 1
            + len(self.LIGHT_INTENSITY_THRESHOLDS)
            + 1
        )
    ...
    def _discretize(self, value: float, thresholds: Tuple[float, ...]) -> int:
        threshold_values = np.asarray(thresholds, dtype=np.float64)
        return int(np.searchsorted(threshold_values, float(value), side="left"))

    def context_bin_indices(
        self,
        angular_velocity: float,
        light_intensity: float,
    ) -> Tuple[int, int]:
        angular_speed = abs(float(angular_velocity))
        light_value = float(light_intensity)
        if light_value < 0:
            raise ValueError("light_intensity must be >= 0 for discrete context binning.")

        angular_bin = self._discretize(angular_speed, self.ANGULAR_SPEED_THRESHOLDS)
        light_bin = self._discretize(light_value, self.LIGHT_INTENSITY_THRESHOLDS)
        return angular_bin, light_bin
    ...
    def build_context(
        ...
        """
        x = [1, one_hot(abs(w) bin), one_hot(light bin)]

        Bins use the configured thresholds as inclusive upper bounds:
        angular speed <= 2.5, <= 5, <= 7.5, <= 10, <= 12.5, > 12.5
        light intensity <= 500, <= 1000, <= 3000, <= 6000, > 6000
        """
        angular_bin, light_bin = self.context_bin_indices(
            angular_velocity=angular_velocity,
            light_intensity=light_intensity,
        )

        angular_features = np.zeros(
            len(self.ANGULAR_SPEED_THRESHOLDS) + 1,
            dtype=np.float64,
        )
        light_features = np.zeros(
            len(self.LIGHT_INTENSITY_THRESHOLDS) + 1,
            dtype=np.float64,
        )
        angular_features[angular_bin] = 1.0
        light_features[light_bin] = 1.0
    ...
    def select_action(
        ...
        return {
            "context": context,
            "angular_speed_bin": angular_bin,
            "light_intensity_bin": light_bin,
            "candidates": rows,
    ...
    def step(
        ...
        record = {
            "angular_velocity": context_information["angular_velocity"],
            "light_intensity": context_information["light_intensity"],
            "angular_speed_bin": sel["angular_speed_bin"],
            "light_intensity_bin": sel["light_intensity_bin"],
