    ...
    try:
        while simulation_app._app.is_running() and not simulation_app.is_exiting():
            ...
            if (step + 1) % CHANGE_CONTEXT_EVERY == 0 and step > 0:
                avg_speed = average_motion_history(log_motion_history)
                curr_speed = max(avg_speed["abs_angular_velocity"], avg_speed["linear_velocity"])  # Ensure non-zero speed for logging
                wandb_run.log(
                ...
                if VERBOSE:
                    print(
                        ...
                        f"Light Intensity set to {context['light_intensity']}, Agent Speed set to {curr_speed}"
                    )
